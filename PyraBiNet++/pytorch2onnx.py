...
def pytorch2onnx(model,
    ...
    if verify:
        ...
        sess = rt.InferenceSession(output_file, providers=['CUDAExecutionProvider', 'CPUExecutionProvider'])
        onnx_result = sess.run(
            None, {net_feed_input[0]: img_list[0].detach().numpy()})[0][0]
        np.save('img.npy', img_list[0].detach().numpy())
...
def parse_args():
    ...
    parser.add_argument('--config', default='/home/zzw/mmsegmentation/tmp/fpn_pvt_t_ade20k_40k.py',help='test config file path')
    parser.add_argument('--checkpoint', help='checkpoint file', default='/home/zzw/mmsegmentation/tmp/pyrabinet++.pth')
    parser.add_argument(
        '--input-img', type=str, help='Images for input', default='/home/zzw/mmsegmentation/demo/demo.png')
    parser.add_argument(
        '--show',
        default=True,
        action='store_true',
    ...
    parser.add_argument(
        '--verify', default=True, action='store_true', help='verify the onnx model')
