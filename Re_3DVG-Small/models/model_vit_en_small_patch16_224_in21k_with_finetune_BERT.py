...
class Model(torch.nn.Module):
    def __init__(self, args):
        ...
        self.bert_model = BertModel.from_pretrained("google/mobilebert-uncased").to(device)
    ...
    def forward(self, batch_image_patches, d3_patches, question):
        ...
        tgt_embedding = outputs.last_hidden_state
        ...
        for d3_patches in batch_d3_patches:
            ...
            for d in d3_patches:
                d3_feature = self.d3_emb(
                    torch.as_tensor(np.array([self.word2idx.index(round(float(i), 2)) for i in d])).to(device)).to(device)
    ...
    def patches_padding(self, max_length, image_query):
        ...
        image_qurey_paded = torch.concat(
            (image_query, torch.zeros([(max_length - image_query.shape[0]), 384]).to(device)), dim=0)
    ...
    def d3_padding(self, max_length, d3_embedding):
        d3_embedding_paded = torch.concat(
