...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=1152, nhead=8, num_encoder_layers=6,
    ...
    def forward(self, src, src_key_padding_mask, tgt_key_padding_mask, tgt, src_pos, tgt_pos):
        # ---------------------------------------------------------------
        # 这一部分是把图片特征送入了encoder，不需要的时候就注释掉
        # ---------------------------------------------------------------
        ...
        # mask = mask.flatten(1)
        memory = self.encoder(src, src_key_padding_mask=src_key_padding_mask, pos=src_pos)
        hs = self.decoder(tgt, memory, tgt_key_padding_mask=tgt_key_padding_mask,
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, src,
        ...
        Returns:
            output: shape=(L,N,E_dim)
        """
        output = src

        for layer in self.layers:
            output = layer(output, src_attn_mask=src_attn_mask,
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        Args:
            tgt: shape=(L2,N,E_dim)
            memory: shape=(L,N,E_dim)
            tgt_attn_mask: shape=(L2,L2)
            memory_mask:
            tgt_key_padding_mask:
            memory_key_padding_mask:
            src_pos:
            tgt_pos: shape=(L2,N,E_dim)
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src,
        ...
        if self.normalize_before:
            return self.forward_pre(src, src_attn_mask, src_key_padding_mask, pos)
        return self.forward_post(src, src_attn_mask, src_key_padding_mask, pos)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
        ...
        super().__init__()
        self.self_attn = nn.MultiheadAttention(d_model, nhead, dropout=dropout).to(device)
        self.multihead_attn = nn.MultiheadAttention(d_model, nhead, dropout=dropout).to(device)
        # Implementation of Feedforward model
        self.linear1 = nn.Linear(d_model, dim_feedforward).to(device)
        self.dropout = nn.Dropout(dropout).to(device)
        self.linear2 = nn.Linear(dim_feedforward, d_model).to(device)
    ...
    def forward(self, tgt, memory,
                ...
                query_pos: Optional[Tensor] = None):
        if self.normalize_before:
            return self.forward_pre(tgt, memory, tgt_attn_mask, memory_mask,
                                    tgt_key_padding_mask, memory_key_padding_mask, pos, query_pos)
        return self.forward_post(tgt, memory, tgt_attn_mask, memory_mask,
                                 tgt_key_padding_mask, memory_key_padding_mask, pos, query_pos)
