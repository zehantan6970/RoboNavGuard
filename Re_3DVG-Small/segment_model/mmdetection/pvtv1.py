...
class ConvBNReLU(nn.Module):
    ...
    def __init__(self, in_chan, out_chan, ks=3, stride=1, padding=1,
        ...
        self.depth_conv = nn.Conv2d(in_chan, in_chan, kernel_size=ks,
                                    stride=stride, padding=padding, groups=in_chan)
        self.point_conv = nn.Conv2d(in_chan, out_chan, kernel_size=1,
                                    stride=1, padding=0, groups=1)
        self.bn = nn.BatchNorm2d(out_chan)
        self.relu = nn.ReLU(inplace=True)

    def forward(self, x):
        feat = self.depth_conv(x)
        feat = self.point_conv(feat)
        feat = self.bn(feat)
        feat = self.relu(feat)
        return feat
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        ...
        self.bn = nn.BatchNorm2d(embed_dim)#zzw
